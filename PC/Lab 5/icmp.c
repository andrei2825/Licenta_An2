	...
	if(count == -1) {
		while(1) {
			...
			*/
			...
			memcpy(&ip_hdr->daddr, &ip_addr, sizeof(struct iphdr));
			...
			*/
			...
			printf("pinged from %d: icmp_seq=%d ttl=%d time=%f\n", rip_hdr->saddr, ricmp_hdr->un.echo.sequence, rip_hdr->ttl, time_elapsed);
		}
	} else {
		while(i < count) {
			i++;
			...
			*/
			...
			/* TODO 1 afișați informațiile relevante din pachetul reply */
			printf("pinged from %d: icmp_seq=%d ttl=%d time=%f\n", ip_hdr->saddr, icmp_hdr->un.echo.sequence, ip_hdr->ttl, time_elapsed);
		}
	}
}
...
void traceroute(int sockfd, struct in_addr ip_addr)
{
	...
	while(time_to_leave <= 30) {
			...
			icmp_hdr->checksum = 0;
			icmp_hdr->un.echo.id = 7;
			icmp_hdr->un.echo.sequence = time_to_leave;
			icmp_hdr->checksum = checksum(icmp_hdr, sizeof(struct icmphdr));
			...
			struct iphdr *rip_hdr = (struct iphdr *)(reply.payload + IP_OFF);
			struct icmphdr *ricmp_hdr = (struct icmphdr *)(reply.payload + ICMP_OFF);
			if(ricmp_hdr->type == 11) {
				struct in_addr ipAddr;
				ipAddr.s_addr = rip_hdr->saddr;
				printf("%d %s ttl=%d %f\n", i, inet_ntoa(ipAddr), time_to_leave, time_elapsed);
				i++;
				time_to_leave++;
			} else {
				printf("%d pinged from %d: icmp_seq=%d ttl=%d time=%f\n", i, ip_hdr->saddr, icmp_hdr->un.echo.sequence, ip_hdr->ttl, time_elapsed);
				return;
			}
	}
}
