...
%   print_integrama(W2).
completare(integ(_, _, [], _), [], _).
   
   
   
   
%     completeHelper1(H, W, First, Rest, Vocab, A, Q_ID);
%     completeHelper2(H, W, First, Rest, Vocab, A, Q_ID);
%     completeHelper3(H, W, First, Rest, Vocab, A, Q_ID).

% completeHelper1(H, W, ((R, C), [(T, D, I)]), Rest, Vocab, T, I):-
%     completare(integ(H, W, Rest, Vocab), Intrebare, Q_ID).
% completeHelper2(H, W, ((R, C), [(T1, D1, I1), (T2, D2, I2)]), Rest, Vocab, T2, I2):-
%     completare(integ(H, W, Rest, Vocab), Intrebare, Q_ID).
% completeHelper3(H, W, ((_, _), _), Rest, Vocab, Intrebare, Q_ID) :-
%     completare(integ(H, W, Rest, Vocab), Intrebare, Q_ID).
