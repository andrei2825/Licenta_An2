...
safe(X/Y/S, Others) :- \+ member(X/_/S, Others), \+ member(_/Y/S, Others).
...
permutari4([A, B, C, D], P) :-  length(P,  4), member(A, P), member(B, P), member(C, P), member(D, P).
...
alternare(_, _, 0, []).
alternare(A, B, L, [A | Res]) :- L2 is L-1, L > 0, alternare(B, A, L2, Res).
...
%% listei.
sublista([], []).
sublista([L1|RestL], [L1|RestS]) :- sublista(RestL, RestS).
sublista([_|RestL], RestS) :- sublista(RestL, RestS).
