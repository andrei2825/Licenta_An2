...
(define (compute-length get-line-segment get-start-point get-end-point)
  (let ((l (get-line-segment))) (let ((x (get-start-point l)) (y (get-end-point l)))
    (sqrt (+ (sqr (- (car y) (car x))) (sqr (- (cdr y) (cdr x))))))))
...
(define (compute-f-with-step f a b step)
  (let custom ((b b) (result '()))
    (if (> a b)
        result
        (custom (- b step) (cons (f b) result)))))
...
(define (num-concat x y)
  (let ((+ string-append) (x (number->string x)) (y (number->string y)))
  (string->number (+ x y))));; Nu ștergeți această linie.
...
(define (compute-perimeter points)
  (let ((p1 (car points)))
    (let iter ((points points) (dist 0))
      (if (null? (cdr points))
          (+ dist (distance (car points) p1))
          (iter (cdr points) (+ dist (distance (car points) (cadr points))))))))
