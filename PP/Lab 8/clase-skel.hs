...
emptyBoard = Board $ replicate 9 Space
...
place c pos@(x, y) (Board board) = Board $ take p board ++ [cell] ++ drop (p + 1) board
    where
        p = 3 * x + y
        cell = case c of 
            'X' -> X
    ...
    where 
        show (Board board) = first ++ "\n" ++ middle ++ "\n" ++ last
            where 
                ...
                middle = concatMap show $ take 3 $ drop 3 board
                last = concatMap show $ drop 6 board
{-
...
instance Ord TicTacToe
    where 
       (Board b1) <= (Board b2) = length (filter(/= Space) b1) <= length (filter (/= Space) b2)
...
class (Ord a) => PQueue pq a where
    ...
    toList q
        | isEmpty q = []
        | otherwise = fromJust (top q) : (toList (pop q))
...
instance (Ord a) => PQueue ListPQ a where
    ...
    insert n (LPQ queue) = LPQ $ insert' n queue
        where
            insert' n [] = [n]
            insert' n q@(x:xs)
                | (fst n) >= (fst x) = n:q
...
merge node1@(rank1 val1 left11 right1) node2@(rank2 val2 left2 right2) 
