...
inorder :: BST a -> [a]
inorder BSTNil = []
inorder (BSTNod value left right) = (inorder left) ++ [value] ++ (inorder right) 

mapTree :: (a -> b) -> BST a -> BST b
mapTree f BSTNil = BSTNil
mapTree f (BSTNod value left right) = BSTNod (f value) (mapTree f right) (mapTree f left)

foldlTree :: (b -> a -> b) -> b -> BST a -> b
foldlTree f acc BSTNil = acc
foldlTree f acc (BSTNod value left right) = foldlTree f (foldlTree f (f acc value) left) right
