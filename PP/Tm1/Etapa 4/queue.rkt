...
(define (rotate left right Acc)
  (if (stream-empty? left)
      ...
      (stream-cons (stream-first left) (rotate (stream-rest left) (stream-rest right) (stream-cons (car right) Acc)))))
...
(define (enqueue x q)
  (let ((q (match q [(queue left right size-l size-r) (struct-copy queue q [right (foldr cons right (list x))] [size-r (+ size-r 1)])])))
    (if (> (queue-size-r q) (queue-size-l q))
        (match q [(queue left right size-l size-r) (struct-copy queue q [left (rotate left right empty-stream)] [right '()] [size-l (+ size-l size-r)] [size-r 0])])
        q)))
...
(define (dequeue q)
  (if (stream-empty? (queue-left q))
      (match q [(queue left right size-l size-r) (struct-copy queue q [left (stream-rest (rotate left right empty-stream))] [right '()] [size-l (- (+ size-l size-r) 1)] [size-r 0])])
      (if (= (queue-size-r q) (queue-size-l q))
          (match q [(queue left right size-l size-r) (struct-copy queue q [left (stream-rest (rotate left right empty-stream))] [right '()] [size-l (- (+ size-l size-r) 1)] [size-r 0])])
          (match q [(queue left right size-l size-r) (struct-copy queue q [left (stream-rest left)] [size-l (- size-l 1)])]))))
...
(define (top q)
  (if (stream-empty? (queue-left q))
      (stream-first (queue-left (match q [(queue left right size-l size-r) (struct-copy queue q [left (stream-rest (rotate left right empty-stream))] [right '()] [size-l (- (+ size-l size-r) 1)] [size-r 0])])))
