...
(define (min-tt counters)
  (helper-tt counters +inf.0 0))

(define (helper-tt L min index)
  (cond
    [(null? L) (list* index min)]
    [(<(counter-tt (car L)) min) (helper-tt (cdr L) (counter-tt (car L)) (counter-index (car L)))]
    [else (helper-tt (cdr L) min index)]))
...
(define (add-to-counter C name n-items)
  (struct-copy counter C [tt (+(counter-tt C) n-items)] [queue (append (counter-queue C) (list (list* name n-items)))]))
  ...
  (if (null? requests)
      ...
      (match (car requests)
        ...
        [(list name n-items)         (cond
                                       [(and (= (car (min-tt (list C1 C2 C3 C4))) 1) (<= n-items ITEMS)) (serve (cdr requests) (add-to-counter C1 name n-items) C2 C3 C4)]
