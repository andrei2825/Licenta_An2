...
(define add-to-counter
  (lambda (C)
    (lambda (name)
      (lambda (n-items)
        (struct-copy counter C [tt (+(counter-tt C) n-items)] [et (if (null? (counter-queue C))
                                                                      (+ (counter-et C) n-items)
                                                                      (counter-et C))] [queue (append (counter-queue C) (list (list* name n-items)))])))))
...
(define (min-tt counters)
  (min-helper counter-tt counters +inf.0 0))  ; folosind funcția de mai sus
(define (min-et counters)
  (min-helper counter-et counters +inf.0 0)) ; folosind funcția de mai sus
...
(define (remove-first-from-counter C)
  (if (= (length (counter-queue C)) 1)
      ...
      (struct-copy counter C [tt (tt-calculator (cdr (counter-queue C)))] [et (cdr (car (cdr (counter-queue C))))] [queue (cdr (counter-queue C))])))

(define (tt-calculator queue)
  (if (empty? queue)
      0
      (+ (cdr (car queue)) (tt-calculator (cdr queue)))))
  ...
  (if (null? requests)
      ...
      (match (car requests)
        [(list 'delay index minutes) (if (>= (length fast-counters) index)
                                         (serve (cdr requests) (list-set fast-counters (- index 1) ((et+ ((tt+ (list-ref fast-counters (- index 1))) minutes)) minutes)) slow-counters)
                                         (serve (cdr requests) fast-counters (list-set slow-counters (- (- index 1) (length fast-counters)) ((et+ ((tt+ (list-ref slow-counters (- (- index 1) (length fast-counters)))) minutes)) minutes))))] 
        ...
        [(list name n-items)         (cond
                                       [(and (<= n-items ITEMS) (<= (car (min-tt (append fast-counters slow-counters))) (length fast-counters))) (serve (cdr requests) (list-set fast-counters (- (car (min-tt (append fast-counters slow-counters))) 1) (((add-to-counter (list-ref fast-counters (- (car (min-tt (append fast-counters slow-counters))) 1))) name) n-items)) slow-counters)]
                                       [(and (<= n-items ITEMS) (> (car (min-tt (append fast-counters slow-counters))) (length fast-counters))) (serve (cdr requests) fast-counters (list-set slow-counters (- (- (car (min-tt slow-counters)) 1) (length fast-counters)) (((add-to-counter (list-ref slow-counters (- (- (car (min-tt slow-counters)) 1) (length fast-counters)))) name) n-items)))]
                                       [else (serve (cdr requests) fast-counters (list-set slow-counters (- (- (car (min-tt slow-counters)) 1) (length fast-counters)) (((add-to-counter (list-ref slow-counters (- (- (car (min-tt slow-counters)) 1) (length fast-counters)))) name) n-items)))])]
        [(list remove-first)         (if (<= (car (min-et-positive (append fast-counters slow-counters))) (length fast-counters))
                                         (if (not (check-zero? (append fast-counters slow-counters)))
                                             (serve (cdr requests) (list-set fast-counters (- (car (min-et-positive fast-counters)) 1) (remove-first-from-counter (list-ref fast-counters (- (car (min-et-positive fast-counters)) 1)))) slow-counters)
                                             (serve (cdr requests) fast-counters slow-counters))
                                         (if (not (check-zero? (append fast-counters slow-counters)))
                                             (serve (cdr requests) fast-counters (list-set slow-counters (- (- (car (min-et-positive slow-counters)) 1) (length fast-counters)) (remove-first-from-counter (list-ref slow-counters (- (- (car (min-et-positive slow-counters)) 1) (length fast-counters))))))
                                             (serve (cdr requests) fast-counters slow-counters)))])))
...
(define (min-et-positive-helper f L min index)
  (cond
    [(null? L) (list* index min)]
    [(and (and (<(f (car L)) min) (> (f (car L)) 0)) (> (length (counter-queue (car L))) 0)) (min-et-positive-helper f (cdr L) (f (car L)) (counter-index (car L)))]
    [else (min-et-positive-helper f (cdr L) min index)]))

(define (min-et-positive counters)
  (min-et-positive-helper counter-et counters +inf.0 0))

(define (check-zero? counters)
  (or (null? counters)
      (and (= (length (counter-queue (car counters))) 0) (check-zero? (cdr counters))))) 