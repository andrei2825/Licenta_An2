...
(define (queue-empty? q)
  (if (and (and (empty? (queue-left q)) (empty? (queue-right q))) (and (= (queue-size-l q) 0) (= (queue-size-r q) 0)))
      #t
      #f))
...
(define (enqueue x q) 
  (match q [(queue left right size-l size-r) (struct-copy queue q [right (foldr cons right (list x))] [size-r (+ size-r 1)])]))
...
(define (dequeue q)
  (if (empty? (queue-left q))
      (match q [(queue left right size-l size-r) (struct-copy queue q [left (cdr (foldl cons left (queue-right q)))] [size-l (- (queue-size-r q) 1)] [right '()] [size-r 0])])
      (match q [(queue left right size-l size-r) (struct-copy queue q [left (cdr left)] [size-l (- size-l 1)])])))
...
(define (top q)
  (if (empty? (queue-left q))
      (car (queue-left (match q [(queue left right size-l size-r) (struct-copy queue q [left (foldl cons left (queue-right q))] [size-l (queue-size-r q)] [right '()] [size-r 0])])))
      (car (queue-left q))))
