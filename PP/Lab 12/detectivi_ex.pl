...
%% au_pusca(-ListaNume)
au_pusca(ListaNume) :-
    % forall(member(X, ListaNume), inarmat(X, pusca)).
    findall(X, (inarmat(X, pusca)), ListaNume).
check2:- tests([
...
au_arma(Arma, ListaNume) :- 
    findall(X, inarmat(X, Arma), ListaNume).

%% au_marca/2
%% au_marca(+Arma, -ListaNume)
au_marca(Marca, ListaNume) :- 
    findall(X, conduce(X, Marca), ListaNume).
...
%% arme_bmw(-ListaArme)
arme_bmw(ListaArme) :- 
    bagof(A, X^suspect(X:bmw:A), ListaArme). 
...
arme_marca(Marca, ListaArme) :- 
    setof(A, X^suspect(X:Marca:A), ListaArme).
...
marci_arma_unica(ListaMarci) :- 
    findall(M, arme_marca(M, [_]), ListaMarci).
...
suspect1(Nume:Marca:Arma) :- 
    suspect(Nume:Marca:Arma),
    arme_marca(Marca, A),
    forall(member(X, A), au_arma(X, [_,_|_])).
